...
### LSTM Woodbridge Bidirectional recurrent_dropout + 256 batchs + Masking after 10 executions + maxlen from 10 to 1000 + first 4 symbols removed
...
library("tibble")
library("ggdark")
files <- list.files(path = "../results/",pattern="results_test_ctu19-lstm_endgame_bi_recurrent_drop-upsample-cswitch_ctu13-\\d+")


results_lstm_bi_recurrent_drop <- lapply(files, function(x) 
                read_csv(paste("../results/",x,sep=""))   
                %>% add_column(maxlen=as.integer(str_replace(string = x ,pattern = ".*-([0-9]+)-[0-9]+.csv","\\1") ))
                )

results_lstm_bi_recurrent_drop <-do.call(rbind,results_lstm_bi_recurrent_drop)

results_lstm_bi_recurrent_drop  %>% arrange(maxlen) %>% filter(metric %in%  c("Balanced Accuracy","F1","Precision","Sensitivity","Specificity")) %>%
  ggplot() +
  labs(title="CTU19 [Train], CTU13 [Test] LSTM Woodbridge Bi Recurrent Drop [10 executions]",
        subtitle="Masking, 256 batch, first 4 symbols removed\nMaxlen from 10 to 1000 ")+
  geom_boxplot(aes(x=as.factor(maxlen),y=value,fill=as.factor(maxlen)),color='darkgray')+
 # geom_point(aes(x=as.factor(maxlen),y=value,color=as.factor(maxlen)),color='black')+
...
results<-rbind(results_lstm %>% add_column(model="lstm"),
               ...
               results_lstm_bi_recurrent_drop %>% add_column(model="lstm_bi_rdrop") 
               )

results %>% filter(metric %in%  c("Balanced Accuracy","F1","Precision","Sensitivity","Specificity")) %>%
  ggplot() +
  labs(title="CTU19 [Train] CTU13 [Test] LSTM variants with upsampling [10 executions]",

...
```{r}
workdir="/home/harpo/hostdir/dga-wb-r/datasets/samples/"

train_ex<-readr::read_csv(paste0(workdir,"CTU19A-first4-removed.csv"))
train_ex_normal <-train_ex %>% filter(LabelName == "Normal")
train_ex_normal_aug<-augment_dataset(train_ex_normal,
                                     (train_ex %>% nrow*2 - train_ex_normal %>% nrow))
train_ex <- rbind(train_ex,train_ex_normal_aug)
...
readr::write_csv(train_ex,file=paste0(workdir,"CTU19A-first4-removed-augmentedx2.csv"))

CTU19A_augmented<-readr::read_csv(paste0(workdir,"CTU19A-first4-removed-augmentedx2.csv"))

CTU19A_augmented %>% group_by(source) %>% summarise(n=n())
CTU19A_augmented %>% group_by(LabelName) %>% summarise(n=n())
```

...
class Dataset(persistent.Persistent):
    ...
    def generate_biargus(self):
        ...
        try:
            argus_path = Popen('bash -c "type argus"', shell=True, stdin=PIPE, stdout=PIPE).communicate()[0].split()[0]
        except IndexError:
    ...
    def generate_binetflow(self):
        ...
        ra_path = Popen('bash -c "type ra"', shell=True, stdin=PIPE, stdout=PIPE).communicate()[0].split()[0]
        if ra_path:
